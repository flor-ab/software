...
#| code-fold: true
#animación gráfico
tabla_pg4_censoxvotos <- tabla_pg4_censoxvotos |> 
  mutate(anno = as.integer(anno))

# Crear el gráfico animado
gplot <- ggplot(tabla_pg4_censoxvotos) +
  aes(x = anno, y = porc, color = comunidad_autonoma) +
  geom_line(aes(linetype = as.factor(codigo_ccaa)), size = 0.7, alpha = 0.8) +
  scale_x_continuous(
    breaks = seq(2007, 2020, by = 1), 
    labels = seq(2007, 2020, by = 1)
  ) +
  scale_color_viridis_d() +
  labs(
    title = "Relación entre Censo y Voto",
    x = "Año", 
...
anim_save("Relación_Censo_Voto.gif", gplot)
```
...
tabla_pg4_censoxvotos <- 
  ...
  mutate(mean_porc = mean(porc),
         tipos_ccaa = case_when(
           ...
           mean_porc > 0.6 ~ "Porc > 0.6 y < 0.71",
           TRUE ~ "Porc <= 0.6"
         ))

# Graficar los datos
ggplot(tabla_pg4_censoxvotos) +
  geom_line(aes(x = anno, y = porc, color = comunidad_autonoma), size = 1, alpha = 0.8) +
  scale_x_continuous(
    breaks = seq(2007, 2018, by = 3), 
    labels = seq(2007, 2018, by = 3)
  ) +
  scale_color_viridis_d() +  # Utilizar la paleta viridis
  labs(
    title = "Relación entre Censo y Voto por Comunidad Autónoma",
    x = "Año", 
    y = "Porcentaje de Voto",
    color = "Comunidad Autónoma"
  ) +
  facet_wrap(~ tipos_ccaa) +  # Dividir en facetas por tipo de porcentaje
  theme_minimal() +
  theme(
    legend.title = element_text(size = 10),
    legend.text = element_text(size = 9)
...
### Diferencia entre zonas rurales de urbanas.

```{r}
#| code-fold: true
#diferenciar zonas rurales de urbanas
...
resultados_rurales <- tabla_maestra |> 
  filter(zona == "rural") |> 
  group_by(siglas) |> 
  summarise(votos_totales = sum(votos)) |> 
  mutate(porcentaje_votos_rural = (votos_totales / sum(votos_totales)) * 100)

resultados_urbanos <- tabla_maestra |> 
  filter(zona == "urbano") |> 
  group_by(siglas) |> 
  summarise(votos_totales = sum(votos)) |> 
  mutate(porcentaje_votos_urbana = (votos_totales / sum(votos_totales)) * 100)

resultados_comparados <- resultados_rurales |> 
  inner_join(resultados_urbanos, by = "siglas") |> 
  select(siglas, porcentaje_votos_rural, porcentaje_votos_urbana) |> 
  mutate(votos_totales = porcentaje_votos_rural + porcentaje_votos_urbana)

resultados_top10 <- resultados_comparados |> 
  arrange(desc(votos_totales)) |> 
  head(10)

datos_procesados <- resultados_top10 |> 
  pivot_longer(cols = c(porcentaje_votos_rural, porcentaje_votos_urbana), 
               names_to = "zona", 
               values_to = "porcentaje_votos")
#graficar
datos_procesados |> 
  ggplot(aes(x = reorder(siglas, -porcentaje_votos), y = porcentaje_votos, fill = zona)) +
  geom_bar(stat = "identity", position = position_dodge()) + 
  scale_fill_manual(values = c("#D2691E", "#B4CDCD"),
                    labels = c("Rural", "Urbana")) + 
  labs(
    title = "Distribución del Voto por Zonas: Rural vs Urbana",
    x = "Partido",
    y = "Porcentaje de votos",
    fill = "Zona",
  ) +
...
La diferencia entre las zonas rurales y urbanas no parece ser tan marcada en términos generales

Mientras los partidos como PP y PSOE tienen una fuerte presencia en ambas zonas, en las zonas rurales el PP obtiene una ligera ventaja. En cambio, partidos más pequeños como VOX o ERC tienen una representación más significativa en ciertas áreas específicas, pero no dominan ninguna zona.
