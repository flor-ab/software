...
## [**Votos a candidatura en los municipios grandes**]{.hl-green}
...
## [**Evolución del Porcentaje de Voto en Relación al Censo por Comunidad Autónoma**] {hl.green}
...
## [**¿Es cierto que determinados partidos ganan en las zonas rurales?**] {hl.green}
...
## [**Resultados de las encuestas**]{.hl-green}
...
## [**Precisión de la participación estimada**]{.hl-green}
...
## [**Resultados en el Congreso de los Diputados**]{.hl-green}
...
<span style="color: orange;">*¿Cómo repartimos los 248 restantes?*</span>
...
# Preparación de los datos para los escaños
...
orden_provincias_escannos <-
  escannos_provincia_anno |> 
  filter(Año == 2019) |> 
  distinct(Provincia, `Número de escaños`) |> 
  arrange(`Número de escaños`) |> 
  select(Provincia) |> 
  pull()


# Gráfico distribución escaños por provincia (agg): Tenemos un conteo de cuántas provincias tienen cada escaño

graf_escannos_prov_t <-   
  escannos_provincia_anno |> 
  filter(Año == 2019) |> 
  mutate(Provincia = factor(Provincia, levels = orden_provincias_escannos, ordered = TRUE)) |> 
  group_by(`Número de escaños`) |> 
  summarise(cantidad = n())

 mediana <- 
   escannos_provincia_anno |> 
  filter(Año == 2019) |> 
   summarise(mediana = median(`Número de escaños`)) |> 
   pull()

# Graficar el histograma con la línea de la mediana
graf_escannos_prov <- 
  ggplot(graf_escannos_prov_t, aes(x = `Número de escaños`, y = cantidad)) +
  geom_bar(stat = "identity", fill = "#064622", alpha = 0.6) +
  geom_vline(xintercept = mediana, linetype = "dashed", color = "#064622", size = 1) +
  annotate("text", x = mediana, y = max(graf_escannos_prov_t$cantidad), 
           label = paste("Mediana:", mediana, "escaños"), color = "#064622", vjust = -0.5) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    strip.text = element_text(face = "bold",
    ...
    plot.title = element_text(face = "bold",
                              ...
                              margin = margin(b = 10)),
    panel.grid = element_blank()) +
  scale_y_continuous(breaks = seq(0, max(graf_escannos_prov_t$cantidad), by = 1)) +
  labs(title = "Número de provincias / Número de escaños",
    x = "Núm. escaños",
    y = "Núm. provincias")
...
# Gráfico distribución de los escaños por provincia, separamos de acuerdo a la mediana del gráfico anterior

# Mas de 5 escaños

grafico_escannos_provincia_mas5 <-
  escannos_provincia_anno |> 
  filter(Año == 2019 & `Número de escaños` > 5) |> 
  mutate(Provincia = factor(Provincia, levels = orden_provincias_escannos, ordered = TRUE)) |> 
  ggplot() +
  geom_col(aes(x = Provincia, y = `Número de escaños`), size = 1, fill = "#064622", alpha = 0.55) +
  coord_flip() +
    theme_minimal() +
  guides(color = "none") +
  labs(subtitle = "Más de 5 escaños") +
...
grafico_escannos_provincia_menos5 <-
  escannos_provincia_anno |> 
  filter(Año == 2019 & `Número de escaños` <= 5) |> 
  mutate(Provincia = factor(Provincia, levels = orden_provincias_escannos, ordered = TRUE)) |> 
  ggplot() +
  ...
  coord_flip() +
    theme_minimal() +
  guides(color = "none") +
  labs(subtitle = "Menos de 5 escaños") +
  theme(panel.grid = element_blank(),
        axis.title = element_blank(),
        axis.text.y = element_text(size = 7, face = "bold"),
        plot.title = element_text(face = "bold",
...
grid.arrange(grafico_escannos_provincia_mas5, grafico_escannos_provincia_menos5, ncol = 2, 
             top = textGrob("Escaños por circunscripción electoral
...
![](images/clipboard-1347468649.png)
</div>
...
Donde el <span style="color: orange;">índice $i$ varía desde 1 hasta el número total de partidos en la circunscripción</span> (que cumplen la norma anterior)

Y el <span style="color: orange;">índice $j$ varía desde 1 hasta el número de escaños a asignar en la provincia</span>
...
![](images/clipboard-1848552451.png)

Tenemos que dividir los votos de los partidos entre todos los números entre 1 y el número de escaños, que son 1, 2 y 3.

Una vez tenemos estos cocientes, se seleccionan tantas casillas como escaños podemos asignar, de forma que las casillas seleccionadas son las que tienen <span style="color: orange;">cocientes más altos (señaladas en naranja)</span>.
...
- :chart_with_downwards_trend: **Participación Electoral**:  
  En torno a **2015**, la participación disminuyó notablemente, pero se _**recuperó hacia 2017-2018**_, mostrando fluctuaciones importantes.  

- :map: **Diferencias Regionales**:  
  Algunas comunidades destacan con una participación cercana al **80%**, mientras que otras alcanzan mínimos del **50%**, evidenciando _**grandes variaciones**_.  

- :arrows_counterclockwise: **Cambio Político**:  
  Desde **2008**, el _**bipartidismo**_ ha dado paso a una mayor fragmentación con la aparición de **Podemos**, **Vox**, y otros partidos. Esto dificulta conseguir _**mayorías absolutas**_, forzando a los partidos a buscar **coaliciones**.  

- :cityscape: **Municipios Grandes**:  
  Los _**partidos tradicionales**_ siguen siendo los más populares, pero desde **2015**, los _**nuevos partidos**_ han ganado _**progresivamente apoyo**_, especialmente en áreas urbanas.  
